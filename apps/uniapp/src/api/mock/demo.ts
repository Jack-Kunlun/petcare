...
import type { User } from "@/api/globals";

declare global {
  interface Window {
    PetDemo: typeof PetDemo;
    StoreDemo: typeof StoreDemo;
    UserDemo: typeof UserDemo;
    runMockDemo: typeof runMockDemo;
  }
}
...
export class UserDemo {
  ...
  // 更新用户信息
  static async updateUser(username: string, updateData: User) {
...
if (typeof window !== "undefined") {
  // 在浏览器环境中，可以在控制台调用
  window.runMockDemo = runMockDemo;
  window.PetDemo = PetDemo;
  window.StoreDemo = StoreDemo;
  window.UserDemo = UserDemo;
