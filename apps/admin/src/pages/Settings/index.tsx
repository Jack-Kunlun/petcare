...
function DomainCard<TConfig>({
  ...
  return (
    <article className="flex min-w-0 flex-col rounded-xl border border-border bg-white p-5 shadow-sm transition-[box-shadow,border-color,background-color] duration-200 hover:border-brand-primary/30 hover:shadow-md motion-reduce:transition-none sm:p-6">
      ...
      <div className="mt-auto flex flex-wrap items-center gap-3 pt-5">
        {canEdit ? (
          <Link
            to={editPath}
            className="inline-flex min-h-11 cursor-pointer items-center justify-center gap-2 rounded-lg border border-brand-primary px-4 py-2 font-semibold text-brand-primary outline-none transition-colors duration-200 hover:bg-page-background active:bg-slate-100 focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 motion-reduce:transition-none"
          >
            编辑配置
        ...
        {current ? (
          <Link
            ...
            className="inline-flex min-h-11 cursor-pointer items-center justify-center rounded-lg px-3 py-2 font-medium text-text-secondary outline-none transition-colors duration-200 hover:bg-page-background active:bg-slate-100 focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 motion-reduce:transition-none"
          >
            查看发布历史
          </Link>
        ) : null}
  ...
  return (
    <div aria-label="正在加载系统设置" className="grid gap-4 lg:grid-cols-3">
      {[0, 1, 2].map((item) => (
        <div
          key={item}
          className="h-72 rounded-xl bg-slate-200 animate-[pc-skeleton-shimmer_220ms_linear_infinite] motion-reduce:animate-none"
        />
      ))}
    </div>
  );
...
export default function Settings() {
  ...
  return (
    <section className="mx-auto w-full max-w-[1280px] text-text-primary">
      ...
      {overviewQuery.isError ? (
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5 text-red-950">
          ...
          <button
            ...
            className="mt-4 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border border-red-700 px-4 py-2 font-semibold outline-none transition-colors duration-200 hover:bg-red-100 active:bg-red-100 focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400 motion-reduce:transition-none"
          >
