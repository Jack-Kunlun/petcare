...
function parseForm(form: FeeFormState): { config: FeeConfig | null; errors: SettingsFieldErrors } {
  ...
  if (platformCommissionBps === null) {
    errors.platformCommissionBps = "最多保留两位小数";
  } else if (platformCommissionBps > 5000) {
    errors.platformCommissionBps = "平台佣金不能超过 50%";
  }

  if (rewardServiceFeeCents === null) {
    errors.rewardServiceFeeCents = "最多保留两位小数";
  }
  ...
  if (minimumWithdrawalFeeCents === null) {
    errors.minimumWithdrawalFeeCents = "最多保留两位小数";
  }
...
const inputClass =
  "mt-1.5 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 pr-14 text-[16px] text-slate-950 outline-none transition-colors duration-200 focus-visible:border-blue-800 focus-visible:ring-2 focus-visible:ring-blue-800/20 motion-reduce:transition-none sm:text-sm";
  ...
  return (
    <fieldset className="rounded-xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <p className="mb-5 text-slate-600">
        比例以整数万分比、金额以整数分保存，界面按百分比和人民币元展示。
      </p>
      <div className="grid gap-5 sm:grid-cols-2">
        <FeeField
          field="platformCommissionBps"
          label="平台佣金"
          suffix="%"
          value={form.platformCommissionBps}
          error={error("platformCommissionBps")}
          onChange={(value) => update("platformCommissionBps", value)}
          onBlur={() => setTouched((current) => new Set(current).add("platformCommissionBps"))}
        />
        <FeeField
          field="rewardServiceFeeCents"
          label="悬赏服务费"
          suffix="元"
          value={form.rewardServiceFeeCents}
          error={error("rewardServiceFeeCents")}
          onChange={(value) => update("rewardServiceFeeCents", value)}
          onBlur={() => setTouched((current) => new Set(current).add("rewardServiceFeeCents"))}
        />
        <FeeField
          ...
          onBlur={() => setTouched((current) => new Set(current).add("withdrawalFeeBps"))}
        />
        <FeeField
          field="minimumWithdrawalFeeCents"
          label="最低提现手续费"
          suffix="元"
          value={form.minimumWithdrawalFeeCents}
          error={error("minimumWithdrawalFeeCents")}
          onChange={(value) => update("minimumWithdrawalFeeCents", value)}
          onBlur={() => setTouched((current) => new Set(current).add("minimumWithdrawalFeeCents"))}
        />
      </div>
    </fieldset>
  );
...
function FeeField({
  ...
  return (
    <label className="font-medium text-slate-800">
      ...
      {error ? (
        <span id={errorId} role="alert" className="mt-1 block text-sm font-normal text-red-700">
          {error}
        </span>
      ) : null}
    </label>
