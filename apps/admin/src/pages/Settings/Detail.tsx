...
export default function SettingsDetail() {
  ...
  const copyDisabled = !draftQuery.isSuccess || Boolean(draftQuery.data);
  ...
  function renderCopyAction(top = false) {
    return (
      <PermissionGate
        all={[settingsMeta.editPermission, "system.publish"]}
        fallback={
          top ? undefined : (
  ...
  if (!version) {
    return (
      <section className="mx-auto w-full max-w-[1024px]">
        ...
        {!versionQuery.isPending && !versionQuery.isError ? (
  ...
  return (
    <>
      <EditorPageLayout
        width="narrow"
        title={`${settingsMeta.label} v${version.version}`}
        description={
          <>
            <span className="mr-2 font-medium text-blue-800">只读历史详情</span>
      ...
      >
        ...
        <FormSection title="需要重新使用这个版本？" actions={renderCopyAction()}>
          <p className="text-slate-600">复制会创建新草稿，不会直接发布或覆盖历史记录。</p>
          {draftQuery.data ? (
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed inset-x-4 top-1/2 z-50 max-h-[90dvh] -translate-y-1/2 overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl outline-none focus-visible:ring-2 focus-visible:ring-blue-800 sm:left-1/2 sm:right-auto sm:w-[min(512px,calc(100vw-32px))] sm:-translate-x-1/2 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <Dialog.Description className="mt-2 leading-6 text-slate-600">
                  将 {settingsMeta.label} v{version.version}{" "}
                  完整复制为新的可编辑草稿。此操作不会发布。
                </Dialog.Description>
              </div>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <PermissionGate all={[settingsMeta.editPermission, "system.publish"]}>
                <button
                  type="button"
