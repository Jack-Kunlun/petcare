      ...
      {version ? (
        <>
          ...
          <section className="mt-5 flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-semibold text-slate-950">恢复为新草稿</h2>
            ...
            <PermissionGate
              all={["website.publish"]}
              fallback={<p className="text-sm text-slate-600">需要 website.publish 权限。</p>}
            >
              <button
                type="button"
                disabled={draftQuery.isPending || draftQuery.isError || !draftQuery.data}
                onClick={() => {
                  setChangeSummary(
                    `从历史版本创建恢复草稿 v${version.businessVersion ?? version.revision}`,
                  );
                  setDialogOpen(true);
                }}
                className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-amber-700 px-5 py-2 font-semibold text-white outline-none hover:bg-amber-800 focus-visible:ring-2 focus-visible:ring-amber-700 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40"
              >
