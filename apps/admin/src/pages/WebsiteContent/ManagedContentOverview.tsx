...
export function ManagedContentOverview({
  ...
  return (
    <section className="mx-auto w-full max-w-[1280px]">
      <header>
        <p className="font-medium text-blue-800">{eyebrow}</p>
        <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">
          {title}
        </h1>
        <p className="mt-2 max-w-[720px] leading-6 text-slate-600">{description}</p>
      </header>
      ...
      {overviewQuery.isError ? (
        ...
        >
          ...
          <h2 className="mt-2 font-semibold">{title}加载失败</h2>
      ...
      {scopedOverview ? (
        <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label={listLabel}>
