  ...
  return (
    ...
    >
      <div className="border-b border-slate-200 px-5 py-4">
        ...
        <p className="mt-1 text-sm text-slate-600">目录只读，API 权限由服务端自动派生。</p>
      </div>
...
/** Lists roles and exposes the read-only server-owned menu permission catalog. */
export default function Rbac() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            维护管理员角色及可分配的菜单、按钮权限；接口权限由服务端目录自动派生。
          </p>
        </div>
      ...
      <div role="tablist" aria-label="角色与目录">
        <button
          type="button"
          role="tab"
          aria-selected={tab === "roles"}
          onClick={() => setTab("roles")}
          className={`min-h-11 border-b-2 px-4 font-semibold ${tab === "roles" ? "border-blue-700 text-blue-800" : "border-transparent text-slate-600 hover:text-slate-950"}`}
        >
          角色管理
        </button>
        <button
          ...
          className={`min-h-11 border-b-2 px-4 font-semibold ${tab === "catalog" ? "border-blue-700 text-blue-800" : "border-transparent text-slate-600 hover:text-slate-950"}`}
        >
          菜单目录
        </button>
      </div>

      {tab === "catalog" && catalogQuery.isPending ? (
        <div className="h-48 animate-pulse rounded-xl bg-slate-200 motion-reduce:animate-none" />
      ) : null}
      {tab === "catalog" && catalogQuery.isError ? (
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5 text-red-950">
          ...
          <button
            ...
            className="mt-3 min-h-11 rounded-lg border border-red-700 px-4 font-semibold hover:bg-red-100"
          >
            重新加载
          </button>
        </div>
      ...
      {tab === "catalog" && catalogQuery.data ? (
        <PermissionCatalog permissions={catalogQuery.data.permissions} />
      ) : null}
