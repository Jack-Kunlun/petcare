...
当前生产能力仍以个人、非商业基线为准。悬赏、订单、SOP、资格写入、支付与结算已获准按路线图分阶段恢复，但默认运行和产品入口必须保持关闭，直到对应纵向链路、外部依赖和发布条件全部通过。
...
- Cycle 5–11 只恢复路线图明确列出的悬赏、订单、SOP、资格写入、支付退款、结算提现和最终商业入口。每次只执行当前队列节点；开发和隔离验收期间默认关闭生产入口。
- 服务者资格、支付和结算必须使用真实可审计状态，禁止用人工假状态、固定成功、模拟回调或测试账务绕过；外部主体、账户、合同和合规责任未齐备时必须失败关闭。
