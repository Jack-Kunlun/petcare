...
PetCare 当前按**个人、非商业、可本地验收的产品原型**维护，已有主线是萌宠课堂、受控社区、本人宠物档案、内容管理和本地运行环境。

服务者实名认证/资格审核、公开悬赏与接单、SOP 履约、微信支付、退款、结算、提现、钱包、B2C 派单及正式对外经营均属于未来商业版设想，当前不实现、不排期，也不得由静态原型页面或测试数据表述为已支持。微信生产登录、生产短信、腾讯云 COS 和小程序发布等外部集成仅作为可选能力，不是个人版本地完成门禁。

当前状态与下一执行范围以[开发路线图](./docs/01-requirements/05-development-roadmap.md)为准；完整商业设想仍保留在 [PRD](./docs/01-requirements/01-prd.md) 中作为未来参考，不能据此判断当前能力。当前版本不得用于撮合真实宠物服务或处理真实资金。
...
- **[当前状态与开发路线图](./docs/01-requirements/05-development-roadmap.md)** - 当前完成度、任务顺序与周期
...
- **[原型规格文档(v47)](./docs/01-requirements/04-prototype-specification.md)**
