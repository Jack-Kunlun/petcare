...
此步骤属于独立的生产权限变更，必须获得明确授权；不随代码发布自动执行，数据库备份桶保持不变。
目标是桶默认私有，仅 `public/*` 允许匿名读，资格材料继续使用显式私有 ACL 与 SSE-COS（`AES256`）。

1. 备份完整 Bucket ACL、Bucket Policy（包括原本不存在的情况）与域名/CDN 配置；盘点全部公开前缀及对象 ACL，
   不把一次抽样当作全桶审计。确认操作账号能恢复配置，Server 账号仍能读写删各自业务前缀。
2. 进入 **腾讯云控制台 → 对象存储 COS → 存储桶列表 → 目标素材桶 → 权限管理 → Policy 权限设置 → 添加策略**。
   向所有用户仅允许 `public/*` 的 `GetObject`、`HeadObject`，不开放列桶、上传、删除或管理权限。
   合并现有策略，不整份覆盖。策略范围和控制台步骤参见[腾讯云存储桶策略说明](https://cloud.tencent.com/document/product/436/18031)。
3. 保留公开读策略后，将桶 ACL 改为私有读写。若有非 `public/` 的公开资源、广域授权或不明确的 CDN 回源行为，先停止迁移。
4. 用无个人数据的唯一临时对象验收：`public/` 匿名 GET/HEAD 正常；`private/provider-qualifications/` 无论显式私有
   或继承默认 ACL，匿名读取均拒绝；签名读写删正常，实际资格上传的加密元数据为 `AES256`。
   默认域名、业务使用的自定义域名与 CDN 分别验证，HTTPS 必须通过正常证书校验，禁止以跳过校验证明通过。
5. 验证 Website、Admin 与客户端的既有公开素材不受影响，删除本次探针并确认不存在；保留脱敏结果与回滚记录。
   任一检查失败即停止开放资格入口，按备份恢复精确 ACL/Policy 后复验公开素材；回滚不能扩大原权限或开放私有材料。

桶策略不代替 CAM 子账号最小权限审计；签名读写成功也不证明账号没有多余权限。
