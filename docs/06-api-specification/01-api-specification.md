...
预支付、退款发起和主动查单共用每账户每分钟 10 次限流。支付状态为 `pending`、`succeeded`、`closed`、`refund_pending`、`refunded`；旧的未支付查询不能覆盖已确认收款，迟到支付结果不能覆盖已转入退款或已退款状态。SOP 的 `canExecute` 与上传/完成写接口均要求 `succeeded`，否则写操作返回 `409 PAYMENT_REQUIRED`。
...
关闭商业开关不阻断存量退款请求、查单或回调；关闭支付配置则所有上述入口返回 404。部分退款、已履约退款、自动取消、异常转账和微信日账单对账尚未实现。无本地退款单的通知返回 404，不擅自补造退款单或判定资金完成，需要后续对账与人工处理流程。
...
后台状态核对由 `WECHAT_PAY_RECONCILIATION_ENABLED` 独立控制，默认关闭。轮询仅 GET 查询已持久单号并复用原始验签、金额匹配和事务迁移，不开放免鉴权查单或“强制成功”接口。队列分类区分查询失败、结果不匹配、支付/退款长期未决、退款异常/关闭、外部退款、已取消订单收款；查询成功后重新判断，终态清除问题，网络错误不覆盖真实资金状态。状态核对不能发现全部无本地单的微信交易，不能代替日账单双向对账、人工差异处理或真实资金验收。
