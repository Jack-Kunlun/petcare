...
以下路由在真实微信支付配置有效，或显式开启 `PAYMENT_SIMULATION_ENABLED=true` 时可用，默认返回 404。模拟模式只允许公开订单标记为 `simulated`（未收款），不调用微信支付，不开放退款或账单核对。请求和响应类型见 `@petcare/shared-types` 的 `OrderPaymentSummary`、`OrderPrepayResponse`。金额和付款 OpenID 来自服务器，不接受客户端提交。

| 方法与路径                                | 权限和行为                                                                                                                 |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `POST /payments/orders/:orderId/prepay`   | 登录且手机号完整的订单主人；还需商业开关开启、订单已确认、服务者资格有效。先持久化支付单再预支付，重复调用复用同一商户单号 |
| `POST /payments/orders/:orderId/simulate` | 仅模拟开关开启时可用；订单主人将已确认订单标记为 `simulated`，明确未收款，不产生微信交易号                                 |
...
预支付、模拟支付、退款发起和主动查单共用每账户每分钟 10 次限流。支付状态为 `pending`、`succeeded`、`simulated`、`closed`、`refund_pending`、`refunded`；`simulated` 只表示演示性流程通过，不代表收款，不能发起退款或进入账单核对。旧的未支付查询不能覆盖已确认收款，迟到支付结果不能覆盖已转入退款或已退款状态。SOP 的 `canExecute` 与上传/完成写接口允许 `succeeded` 或 `simulated`，否则写操作返回 `409 PAYMENT_REQUIRED`。

| 方法与路径                                            | 权限和行为                                                                                                                                                                |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /admin/payments/orders/:orderId/refund`         | `payment.refund_action`；请求 `CreateOrderRefundRequest` 仅接受 5–500 字符原因。只对已取消、已收款且无履约证据/完成记录的悬赏订单发起一次全额原路退款，金额从原支付单冻结 |
| `POST /admin/payments/orders/:orderId/refund/refresh` | `payment.refund_read`；以原退款单号向微信查询并验签、核对后事务更新                                                                                                       |
| `GET /payments/orders/:orderId/refund`                | 仅订单主人可读 `OrderRefundSummary`，不返回管理员原因或付款账户；越权返回 404                                                                                             |
| `POST /payments/wechat/refund-notify`                 | 无登录令牌；原始字节验签、解密、事件类型/商户/原交易/退款单/金额匹配后，状态与通知去重记录同事务提交，返回空 204                                                          |

新增退款权限默认仅超管拥有，不恢复旧订单/财务入口。退款单状态为 `pending`（受理未知）、`processing`、`succeeded`、`abnormal`、`closed`。首次发起保留操作人、原因与时间，同订单唯一退款单，重试不得变更原因或单号；已受理的重复请求只读返回状态。创建退款单即暂停履约，失败或超时不恢复支付状态；成功不可被旧查询覆盖，异常/关闭不可被旧的处理中结果清除。全额退款核对订单退款额与用户现金退款额，优惠券金额不冒充现金到账。

关闭商业开关不阻断存量退款请求、查单或回调；关闭支付配置则所有真实支付入口返回 404。模拟支付单不进入退款、结算或日账核对。部分退款、已履约退款、自动取消和异常转账尚未实现。无本地退款单的通知返回 404，不擅自补造退款单或判定资金完成，需要日账核对与后续人工处理流程。
