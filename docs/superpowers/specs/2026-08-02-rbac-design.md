...
菜单结构不存入数据库不会降低授权安全性，因为菜单只负责导航，Server 的控制器权限声明和 `PermissionGuard` 才是最终安全边界。为防止代码与数据库权限记录漂移，增加以下约束：

- `PermissionCatalogService` 在启动时构建当前目录的稳定版本标识，并检查数据库中是否存在未知权限码；未知记录只产生残留权限告警，不会被授权计算采纳。
- `AuthService.getCurrentUserAuthorization` 只返回当前共享目录中的有效权限码；数据库中残留的旧权限不能通过登录响应或 `PermissionGuard` 获得授权。
- 角色权限替换接口只接受当前目录中的 `menu` 和 `button` 权限码，并拒绝未知码、API 码和不满足父级关系的按钮码。
- 权限码一经发布不得复用为不同语义；共享包测试和代码审查必须保留权限码变更记录。
- CI 在构建前校验权限码、路径、父子关系和隐含 API 引用；部署时先发布兼容目录，再发布使用目录的 Server/Admin 代码。
- `/admin/rbac/catalog` 返回目录版本，Admin 发现版本变化时重新拉取目录，不缓存权限结构到本地持久存储。
- 残留权限和目录版本不一致写入结构化告警，角色授权变更仍写入 `PermissionAuditLog`。

因此，数据库保存的是可追溯的角色授权关系，而不是可修改的菜单定义；即使数据库中存在旧权限记录，默认行为也是拒绝未知权限，而不是扩大授权。
...
- 数据库中残留但不在当前目录的权限不删除数据，只在授权计算中失效并产生告警。
