...
- Default identity is username `admin`, phone `13800138000`, role `super_admin`, status `active`.
...
it("creates one super admin and assigns all permissions", async () => {
  ...
  expect(prisma.users[0]).toMatchObject({ username: "admin", phone: "13800138000" });
...
it("enforces cooldown and hourly send limits", async () => {
  await service.send("13800138000");
  await expect(service.send("13800138000")).rejects.toMatchObject({ status: 429 });
});
```
...
Generate a password with at least 24 random printable characters using Node `crypto.randomBytes`. Write it only to root `.env` as `DEFAULT_ADMIN_PASSWORD`; also write `DEFAULT_ADMIN_USERNAME=admin`, `DEFAULT_ADMIN_PHONE=13800138000`, and `SMS_DEV_CODE=246810`. Do not print the generated password in command output or add it to any tracked file.
...
- password login by `admin` and by `13800138000`;
