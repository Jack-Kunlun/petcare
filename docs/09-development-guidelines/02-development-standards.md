...
PetCare 已完成个人版基线，并只按路线图分阶段恢复明确获准的 C2C 能力。后续工作必须先阅读：

1. [当前状态与开发路线图](../01-requirements/05-development-roadmap.md)，确认任务属于当前执行队列；
2. [个人基线与商业能力分阶段恢复规范](./06-personal-scope-and-code-cleanup.md)，确认历史商业代码的审计、隔离、数据和验收边界；
3. 本文对应章节及其他领域规范，执行具体编码、测试和提交要求。

只有路线图当前 Cycle 明确列出的悬赏、订单与 SOP 工作构成需求；资质外部接入、支付退款、结算提现、钱包和 B2C 派单仍只属于历史或未来设计。禁止从示例、既有页面、接口、Schema 或测试中推导额外任务。

当前首要清理工作包括：移除 Miniapp 与 Website 的商业入口和静态经营文案，移除 Admin 商业导航，隔离默认 Server 商业 API，再删除无消费者的页面、客户端、Mock、资源和专属测试。Prisma 模型、已提交 migration 与运行数据必须作为独立高风险任务处理，不得随普通代码清理删除。

Git 主线、Node/pnpm 版本、技术栈和强制命令以根目录 `AGENTS.md` 与实际仓库配置为准；本文中的旧工作流或商业代码片段如与其冲突，以当前仓库规则为准。
...
- [ ] 工作项是否存在于当前路线图，而不是只存在于未来 PRD 或历史原型？
- [ ] 是否避免重新暴露服务者认证、悬赏、订单、支付、结算、提现等暂停能力？
- [ ] 删除前是否检查路由、调用、资源、测试和数据依赖？
- [ ] 是否把 Schema、migration 和运行数据清理留在独立任务中？
- [ ] 产品文案和演示数据是否只表达当前真实能力？
