...
PetCare 当前只开发个人、非商业、可本地验收的产品原型。后续工作必须先阅读：

1. [当前状态与开发路线图](../01-requirements/05-development-roadmap.md)，确认任务属于当前执行队列；
2. [个人开发版范围与代码清理规范](./06-personal-scope-and-code-cleanup.md)，确认商业残留代码的删除、隔离、数据和验收边界；
3. 本文对应章节及其他领域规范，执行具体编码、测试和提交要求。

服务者认证、悬赏/接单、SOP、支付退款、结算提现、钱包和 B2C 派单等商业示例只展示语法或历史设计，不构成当前需求。禁止从示例、既有页面、接口、Schema 或测试中推导新任务。
