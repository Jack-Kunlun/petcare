...
后台查询启用后立即扫描一次，此后每分钟最多领取 10 张到期支付单，逐张使用 2 分钟数据库租约；网络请求不占用事务锁，本进程不重叠执行。存在本地退款单时只查退款，没有退款单时查支付；继续查询已收款订单以发现外部退款。初始查询间隔 1 分钟，未决单龄超过 5 分钟/1 小时后分别为 5/30 分钟；已收款或待人工核查的单正常间隔为 1 小时。查询失败按 2、4、8…分钟退避，最大 60 分钟，进程重启不重置；关闭或全额退款成功后停止轮询，不能以查询失败代替终态。
