...
审计确认 `UserModule` 当前注册 `POST /users/register`，其实现会：
...
固定假令牌不是有效会话证据，但该端点仍会制造未经验证的账户数据，并向调用方返回误导性的成功响应。它必须在
任何目标环境发布和数据库收缩前作为独立鉴权修复关闭或替换，不能混入删除表的 migration 提交。

## 7. 推荐的后续执行合同

### 7.1 先决条件

1. 关闭旧公共注册端点及无消费者注册/实名客户端契约，并运行认证与账户定向测试；
