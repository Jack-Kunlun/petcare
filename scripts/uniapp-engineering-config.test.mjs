...
test("generated UniApp API definitions stay tool-owned", async () => {
  const generatedDefinitionsPath = "src/api/apiDefinitions.ts";
  const [prettierIgnore, eslintConfigSource, generatedDefinitions] = await Promise.all([
    readFile(resolve(repositoryRoot, ".prettierignore"), "utf8"),
    readFile(resolve(uniappRoot, "eslint.config.mjs"), "utf8"),
    readFile(resolve(uniappRoot, generatedDefinitionsPath), "utf8"),
  ]);

  assert.match(prettierIgnore, /^apps\/uniapp\/src\/api\/apiDefinitions\.ts$/m);
  assert.match(eslintConfigSource, /["']src\/api\/apiDefinitions\.ts["']/);
  assert.match(generatedDefinitions, /auto generated by the alova's vscode plugin/i);
  assert.match(generatedDefinitions, /\/\* eslint-disable \*\//);
});
