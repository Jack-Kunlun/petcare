...
def export_logo_suite(repo_root: Path) -> list[Path]:
    """Export all raster derivatives from the committed curved SVG masters."""
    ...
    admin_directory = repo_root / "apps" / "admin" / "public" / "brand"
    color_symbol_svg = svg_directory / "petcare-symbol-color.svg"
    outputs: list[Path] = []
    ...
    for size in SYMBOL_SIZES:
        ...
        render_svg(color_symbol_svg, output, size, size)
        outputs.append(output)

    admin_directory.mkdir(parents=True, exist_ok=True)
    for variant in ADMIN_VARIANTS:
        for family in ("petcare-logo-stacked", "petcare-symbol"):
            svg_source = svg_directory / f"{family}-{variant}.svg"
            svg_output = admin_directory / svg_source.name
            shutil.copyfile(svg_source, svg_output)
            outputs.append(svg_output)
            aspect_ratio = _svg_aspect_ratio(svg_source)
            for size in HIGH_RES_SIZES:
                width = size
                height = size if family == "petcare-symbol" else round(size / aspect_ratio)
                canonical_output = png_directory / f"{family}-{variant}-{size}.png"
                render_svg(svg_source, canonical_output, width, height)
                outputs.append(canonical_output)
                admin_output = admin_directory / canonical_output.name
                shutil.copyfile(canonical_output, admin_output)
                outputs.append(admin_output)
            compatibility_output = admin_directory / f"{family}-{variant}@2x.png"
            shutil.copyfile(png_directory / f"{family}-{variant}-2048.png", compatibility_output)
            outputs.append(compatibility_output)

    favicon_svg = favicon_directory / "petcare-favicon.svg"
    favicon_directory.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(color_symbol_svg, favicon_svg)
